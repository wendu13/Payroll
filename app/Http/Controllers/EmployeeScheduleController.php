...
class EmployeeScheduleController extends Controller
{
    public function store(Request $request, Employee $employee)
    {
        ...
        try {
            ...
            $scheduleFile = ScheduleFile::create([
                ...
                'days_json' => $daysData, // This will be auto-encoded to JSON due to cast
            ]);

            // 2. Create individual schedule records
            $successCount = 0;
            foreach ($daysData as $day) {
                EmployeeSchedule::create([
                    'employee_id' => $employee->id,
                    'cutoff_schedule_id' => $cutoff->id,
                    'schedule_file_id' => $scheduleFile->id, // Link to the file
                    'date' => $day['date'],
                    'start_time' => $validated['time_in'],
                    'end_time' => $validated['time_out'],
                    'type' => $day['type'], // 'regular' or 'restday'
    ...
    public function viewSchedule(Employee $employee, ScheduleFile $file)
    {
        $data = [
            ...
            'dates'        => $file->days_json, // This is automatically decoded due to cast
        ];

        return response()->json($data);
    }

    // Download JPEG (keeping your existing download method)
    public function download(Employee $employee, $id)
    {
        $file = ScheduleFile::where('employee_id', $employee->id)->findOrFail($id);
        $days = $file->days_json; // No need for json_decode due to cast
